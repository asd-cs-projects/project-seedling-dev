...
// Calls Google Gemini directly. `apiKey` is GEMINI_API_KEY; `model` is the Gemini model id.
const callOpenRouter = async (prompt: string, apiKey: string, model: string, maxTokens = 4096) => {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    console.error(`Gemini attempt ${attempt + 1} failed (${response.status}):`, lastErr);
  ...
  throw new HttpError(lastStatus, `Gemini failed: ${lastErr}`);
};
