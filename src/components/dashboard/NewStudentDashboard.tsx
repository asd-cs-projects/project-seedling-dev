...
const NewStudentDashboard = () => {
  ...
  const firstAttemptResults = useMemo(
    () => testResults.filter(r => !r.is_retake),
    [testResults]
  );

  // Pre-group results by test_id once and assign attempt numbers
  const resultsWithAttempts = useMemo(() => {
    const groups: Record<string, any[]> = {};
    for (const r of testResults) (groups[r.test_id] ??= []).push(r);
    const attemptMap = new Map<string, number>();
    for (const arr of Object.values(groups)) {
      arr.sort((a, b) => new Date(a.completed_at).getTime() - new Date(b.completed_at).getTime());
      arr.forEach((r, i) => attemptMap.set(r.id, i + 1));
    }
    return { attemptMap };
  }, [testResults]);
  ...
  const getSubjectStats = useCallback((subject: Subject) => {
    ...
    const recentScores = [...allRelevant]
      ...
      .map(r => ({
        ...
        score: r.score,
        attemptNumber: resultsWithAttempts.attemptMap.get(r.id) || 1,
      }));

    return { testsTaken: relevantFirstAttempts.length, recentScores };
  }, [firstAttemptResults, testResults, resultsWithAttempts]);
  ...
  const scoreData = useMemo(() => {
    return [...testResults]
      ...
      .slice(-10)
      .map(r => {
        const attemptNumber = resultsWithAttempts.attemptMap.get(r.id) || 1;
        return {
          name: `${(r.tests?.title || 'Test').substring(0, 10)}${attemptNumber > 1 ? ` (#${attemptNumber})` : ''}`,
          fullName: `${r.tests?.title || 'Test'} (Attempt ${attemptNumber})`,
          score: r.score || 0,
        };
      });
  }, [testResults, resultsWithAttempts]);

  const { totalCorrect, totalWrong, totalQuestions, subjectData } = useMemo(() => {
    let tc = 0, tw = 0, tq = 0;
    for (const r of testResults) {
      tc += r.correct_answers || 0;
      tw += r.wrong_answers || 0;
      tq += r.total_questions || 0;
    }
    const subjAcc: Record<string, number> = {};
    for (const r of firstAttemptResults) {
      const s = r.tests?.subject || 'Unknown';
      subjAcc[s] = (subjAcc[s] || 0) + 1;
    }
    return {
      totalCorrect: tc,
      totalWrong: tw,
      totalQuestions: tq,
      subjectData: Object.entries(subjAcc).map(([name, value]) => ({ name, value })),
    };
  }, [testResults, firstAttemptResults]);
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="p-4 bg-muted/30 rounded-xl">
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">ASD ID</p>
                  <p className="text-lg font-semibold mt-1">{profile?.student_id || '-'}</p>
                </div>
                <div className="p-4 bg-muted/30 rounded-xl">
